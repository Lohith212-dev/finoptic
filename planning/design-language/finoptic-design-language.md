...
engineered is precisely what he read as waste. **Read this table before v4.3's.***
...
- ✅ **The KPI tiles and the insight band share one tabbed region**, headline and tabs on the CANVAS above both panes, `Key Insights` default, a `View KPIs ›` footnote at the foot of the band. 🚫 Not one card wrapping both — that was built and taken out (§7, 14.3–14.5). **Except where the screen has one tile: no tabs, no footnote, both stacked** (§7, 17.5).
...
- ✅ **Every figure is the PERIOD value with a YTD byline under it** — strip lane and KPI tile alike, the byline in `.ledger-sub`'s treatment. 🚫 **No tile label may say "YTD" over a figure that moves with the period pill** (§7, 17.3).
- ✅ **A KPI tile carries its trend INLINE and GREY** — 64×22px, right of the figure, only the months the figure was computed from, answering a hover. 🚫 Never accent, never a status colour, and 🚫 **never on a structural count** (`Environments`, `Vendors`, `Active Contracts`) — there is no honest history of those (§7, 17.4, 17.8).
- ✅ **The insight band is at most two POINTERS per column, as a real `<ul>`.** It stays the one INK panel. 🚫 Not paragraphs, and 🚫 not three pointers (§7, 17.6).
