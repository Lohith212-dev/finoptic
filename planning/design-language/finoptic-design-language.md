...
| 19.2 | **A CURVE THROUGH DATA IS MONOTONE CUBIC — Fritsch–Carlson, never Catmull-Rom or a cardinal spline.** 🚫 No smoothing that can overshoot a point. | *"What are your views on making them smooth and curved instead of these sharp lines?"* — yes, but only the kind that cannot lie. An ordinary smoothing spline bulges past a local peak, and on a 76px plot with **no axis to check it against** that bulge reads as a value. Monotone interpolation zeroes the tangent at each turn and clamps it elsewhere, so every high and low in the drawing is a month in the data. `test-metrics.js` proves it on all 139 plots rather than trusting it: swap the tangent rule for the naive one and **84 overshoots** appear. |
