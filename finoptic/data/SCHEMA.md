...
The spend each opportunity's annual saving comes out of. Generated by
`scratchpad/gen-opp-spend.js`, the same shape as the `monthly` block: a script builds
the figure from data the dataset already carries and re-verifies it, rather than it
being hand-typed per opportunity.

It is not a lookup into `categories[]`. `opps[].cat` is a short label ("Cloud",
"Contract", "Licence", …) that does not line up with the eight real spend
categories — "Contract" matches none of them, and "SaaS" / "Licence" would both
collapse onto the one combined "SaaS & licences" total, so several rows would print
the same category-wide figure. That is the same fault round 14 removed from the KPI
tiles (one lane restated across several places), so this generates a real,
opportunity-specific number instead: `eff` is treated as a proxy for how much of the
underlying spend an opportunity actually reaches — a Low-effort item is almost
always a narrowly-scoped, fully-identified pocket of spend (terminate three named
instances, reclaim named seats), so its saving recovers a large share of what it
touches; a High-effort item (a contract renegotiation, a tiering change) reaches a
much larger spend base than it recovers. `RECOVERY` in the generator encodes that as
a share of spend recovered per effort tier — the same shape as `committedRatio` in
`gen-monthly.js`, an authored constant driving a generated, reconciling figure.
...
```jsonc
{
  ...
  // spend is generated by scratchpad/gen-opp-spend.js -- see that file and
  // invariant 20 for what it means and why it isn't a category lookup.
  "opps":[ { "o":"Terminate 3 idle EC2 instances — Alpha production","cat":"Cloud",
             "spend":65,"s":34,"eff":"Low","conf":"High","owner":"S. Menon",
